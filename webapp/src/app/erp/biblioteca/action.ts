...
export async function listarLivros() {
  return prisma.livro.findMany({ include: { exemplares: true } });
}
...
export async function atualizarLivro(id: number, data) {
...
export async function listarEmprestimos() {
  return prisma.emprestimoLivro.findMany({
    include: { exemplar: { include: { livro: true } }, medium: true },
